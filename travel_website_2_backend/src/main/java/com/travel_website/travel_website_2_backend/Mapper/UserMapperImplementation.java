...
//        List<UserDTO.ReservationDto> orders = user.getOrders().stream().map(this::toUserDtoOrderDto).toList();
        return new UserDTO(user.getId(), user.getName(), user.getSurname(), user.getEmail(), user.getTelephone(), user.getCountry(), user.getRole(), user.getUsername(), user.getPassword());
    }
    ...
    private UserDTO.PhotoDto toUserDtoPhotoDto(Photos photo) {
        if (photo == null) {
            return null;
        }
        return new UserDTO.PhotoDto(photo.getId());
    }
}
